...
const NEXT_LEVEL_SCENE_PATH  := "res://niveau3.tscn"
...
func _ready() -> void:
    ...
    set_process(false) # on attend que le joueur bouge (start() appelé par le joueur)
...
func win() -> void:
    ...
    get_tree().change_scene_to_file(NEXT_LEVEL_SCENE_PATH)  # → niveau3
...
func fail() -> void:
    ...
    await get_tree().create_timer(2.0).timeout
    get_tree().change_scene_to_file(FIRST_LEVEL_SCENE_PATH) # → niveau1
